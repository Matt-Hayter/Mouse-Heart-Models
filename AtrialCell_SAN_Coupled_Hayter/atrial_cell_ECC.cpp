...
void atrial_cell::ecc_ODEs(const double &time)
{
    ...
    double I_Na = INa_channel(time);
    double I_Nak = INaK_channel(time);
    double I_to = atrial_config.Ito_Block * Ito_channel(time);
    double I_Kur = atrial_config.IKur_Block * IKur_channel();
    double I_Kss = atrial_config.IKss_Block * IKss_channel();
    double I_Kr = atrial_config.IKr_Block * IKr_channel();
    double I_KACh = atrial_config. IKACh_Block * IKACh_channel();
    double I_K1 = atrial_config.IK1_Block * IK1_channel();
    double I_KCa = atrial_config.IKCa_Block * IKCa_channel();
    double I_Kb = atrial_config.IKb_Block * background_currents(time, 6);
    double I_NCX = INCX_channel();
    double I_Nab = background_currents(time, 1);
    double I_ClCa = background_currents(time, 2);
    double I_Clb = background_currents(time, 3);
    double I_Cap = background_currents(time, 4);
    double I_Cab = background_currents(time, 5);
    double I_CaL = LCC_channel();

    Na_concentrations();
    classic_Ca_handling(time);
    ...
    double I_tot = I_Na_tot + I_Ca_tot + I_K_tot + I_Cl_tot; //[uA/uF]

    // **Update membrane voltage rate**
    ecR->V = -I_tot -I_j; //[V/s]

    //Control of intracellular [K]
    if(atrial_config.fixed_Ki) {
        ecR->Ki = 0;
    } else {
        //intracellular [K] set to fixed or updated by this formula [mM/msec]
        if (atrial_config.K_junction_current) {
            ecR->Ki = -(I_K_tot + I_j) * Cmem / (Vmyo * Frdy);
        } else {
...
double atrial_cell::INa_channel(const double &time)
{
    ...
    if (atrial_config._CaMKII_level_ == 1 && (atrial_config.ACUTE == 1 && time > atrial_config.ACUTE_TIME)) // acute CaMKII effects
    {
        inashift = -3.25;
        alphaCKII = -0.18;
        deltGbarNal_CKII = 3;
    }
    if (atrial_config._LOOP_ == 1) {
        ...
        if (atrial_config.NaVsCaMKIIclamp == 1) {
        delta_loop = (3.0 / (RyRp_OE_mean - RyRp_WT_mean)) * RyRp_OEloop_min - (3.0 / (RyRp_OE_mean - RyRp_WT_mean)) * RyRp_WT_mean;
        }
        if (atrial_config._CaMKII_level_ == 1 && (atrial_config.ACUTE == 1 && time > atrial_config.ACUTE_TIME)) deltGbarNal_CKII = delta_loop; //acute CaMKII effects
    ...
    I_Na_fast_junc = atrial_config.INa_Block * Fjunc_na * GNa * pow(ec.m_na, 3) * ec.h_na * ec.j_na * (V - dependents.Ena_junc);
    I_Na_fast_sl = atrial_config.INa_Block * Fsl_na * GNa * pow(ec.m_na, 3) * ec.h_na * ec.j_na * (V - dependents.Ena_sl);
    } else {
        /////////////////////// old I_Na /////////////////////////////
    ...
    if (atrial_config._CaMKII_level_ == 1 ||
        (atrial_config.ACUTE == 1 && time > atrial_config.ACUTE_TIME)) //weijian, acute and chronic CaMKII effects
        hlss = 1 / (1 + exp((V + 91 - 6.8) / 6.1));

    double tauhl = 600.0; // ms
    ecR.h_l = (hlss - ec.h_l) / tauhl;
    double I_Na_slow_junc = atrial_config.INa_Block * Fjunc_na * GbarNal * pow(ec.m_na, 3) * ec.h_l * (V - dependents.Ena_junc);
    double I_Na_slow_sl = atrial_config.INa_Block * Fsl_na * GbarNal * pow(ec.m_na, 3) * ec.h_l * (V - dependents.Ena_sl);
...
double atrial_cell::INaK_channel(const double &time)
{
    const eccs *ec = &(states.ec);
    double V = ec->V;
    ...
    if (atrial_config._CaMKII_level_ == 1 || (atrial_config.ACUTE == 1 && time > atrial_config.ACUTE_TIME)) IbarNaK = IbarNaK * 0.9; // acute and chronic CaMKII effects
    if (atrial_config.DigitalisFlag == 1) IbarNaK = IbarNaK * 0.5; // 50% block
    if (atrial_config.PLMkoFlag == 1) {
        PLM_PKAp = 1;
        IbarNaK = IbarNaK * 0.8;
    }
    if (atrial_config.StrophFlag == 1) IbarNaK = 0;

    double I_Nak;
    if (!atrial_config.new_INaK) {
        ...
        dependents.I_Nak_junc = atrial_config.INaK_Block * Fjunc_nak * IbarNaK * fnak * _Ko_ / (1 + pow((_KmNai_p / ec->Nai_junc), 4)) / (_Ko_ + Km_Ko_);
        dependents.I_Nak_sl = atrial_config.INaK_Block * Fsl_nak * IbarNaK * fnak * _Ko_ / (1 + pow((_KmNai_p / ec->Nai_sl), 4)) / (_Ko_ + Km_Ko_);
        ...
        double Pnak = 30;
        I_Nak = atrial_config.INaK_Block * Pnak * (zna * JnakNa + zk * JnakK);
...
double atrial_cell::Ito_channel(const double &time)
{
    const eccs *ec = &(states.ec);
    eccs *ecR = &(states_dot.ec);
    ...
    if (atrial_config._CaMKII_level_ == 1 || (atrial_config.ACUTE == 1 && time > atrial_config.ACUTE_TIME)) K_fto = K_fto * 5; // chronic and acute CaMKII effects
...
double atrial_cell::IK1_channel()
{
    ...
    if (atrial_config._CaMKII_level_ == 1 && atrial_config.ACUTE == 0) { //chronic CaMKII effects
...
//INab, I_ClCa, I_Clb, I_Cap, I_Cab
    double atrial_cell::background_currents(const double &time, const int type)
    {
        const eccs *ec = &(states.ec);
        ...
        if (type == 1) {
            ...
            if (atrial_config._CaMKII_level_ == 1 && atrial_config._LOOP_ == 0) GNaB = GNaB * 4; // chronic CaMKII effects
            if (atrial_config.PLMkoFlag == 1) GNaB = GNaB * 48.0 / 20.0;
            dependents.I_Nab_junc = atrial_config.INab_Block * Fjunc_na * GNaB * (V - dependents.Ena_junc);
            dependents.I_Nab_sl = atrial_config.INab_Block * Fsl_na * GNaB * (V - dependents.Ena_sl);
            ...
            double I_Cab_junc = atrial_config.ICab_Block * Fjunc * _GCaB_ * (V - dependents.Eca_junc);
            double I_Cab_sl = atrial_config.ICab_Block * Fsl * _GCaB_ * (V - dependents.Eca_sl);
            double I_Cab = I_Cab_junc + I_Cab_sl;
            //Caffeine effect
            if (atrial_config.caffeine_on && time > atrial_config.caffeine_time) I_Cab *= 0;
...
double atrial_cell::INCX_channel()
{
    ...
    if (atrial_config._CaMKII_level_ == 1 && atrial_config.ACUTE == 0) IbarNCX = 1.5 * IbarNCX; // chronic CaMKII effects
    ...
    double I_NCX_junc = atrial_config.INCX_Block * Fjunc_ncx * IbarNCX * Ka_junc * (s1_junc - s2_junc) / s3_junc;
    ...
    double I_NCX_sl = atrial_config.INCX_Block * Fsl_ncx * IbarNCX * Ka_sl * (s1_sl - s2_sl) / s3_sl;
...
double atrial_cell::LCC_channel()
{
    ...
    double pNa = atrial_config.ICaL_Block * K_Ica * 1.5e-8; // [cm/sec]
    double pCa = atrial_config.ICaL_Block * K_Ica * 5.4e-4; // [cm/sec] - Ca permeability
    double pK = atrial_config.ICaL_Block * K_Ica * 2.7e-7; // [cm/sec]
    ...
    double fckiim2 = atrial_config.max_m2_LCC_CK * LCC_CKp; // Assumes max phos results in 10% mode 2 channels (max LCC_CKp = 1)
...
void atrial_cell::Na_concentrations()
{
    ...
    if (atrial_config.NaClampFlag == 1) ecR->Nai_i = 0; // Na clamp
    else {
        ecR->Nai_i = J_na_slmyo / Vmyo * (Nai_sl - ec->Nai_i); // [mM/msec], Nai_i
    }
}

void atrial_cell::classic_Ca_handling(const double &time)
{
    const eccs *ec = &(states.ec);
    eccs *ecR = &(states_dot.ec);
    ...
    // IP3r Update
    if (atrial_config.IP3R_effect_on) {
...
double atrial_cell::RyR_rel(const double &time, const double *p, double *ptr)
{
    ...
    // Caffeine Inducing
    if (atrial_config.caffeine_on && time > atrial_config.caffeine_time) {
        koSRCa *= 7.5;
    }
...
double atrial_cell::RyR_leak(const double *p)
{
    ...
    double J_SRleak = atrial_config.JSRleak_Block * kleak * (Ca_sr - Ca_junc); // [mmol/L cyt/ms]
...
double atrial_cell::SERCA(const double &time, const double *p)
{
    double Ca_sr = p[0];
    double Cai = p[1];
    ...
    double J_serca = atrial_config.Jserca_Block * Kserca * _Vmax_SERCA_ * (pow((Cai / Kmf), _hillSRCaP_) - pow((Ca_sr / Kmr), _hillSRCaP_))
