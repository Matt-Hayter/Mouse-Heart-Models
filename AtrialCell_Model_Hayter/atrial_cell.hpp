#ifndef ATRIAL_CELL_H
#define ATRIAL_CELL_H
...
class atrial_cell
{
    protected:
        ...
        //Variables for each state, with an index corresponding to it's position within the state vector:
        ...
        const atrial_configuration config; //Contains user's configuration
        ...
        const double FoRT = Frdy / _R / config.temp;
        ...
        const double Qpow = ((config.temp - 295.0) / 10.0);
        ...
        const double freq = 1000/config.BCL; //Basic cycle length
        double I_stim{}, old_stim{}; //Stimulus currents
        double time{}; //Current time for simulation
        double dVdt_prev{}; //Voltage rate for the previous timestep, for use in measurements
        double dVdt_max{}; //Holds max dV/dt
        double V_prev{}; //Potential from prev timestep
        bool take_measurements{false}; //Only take measurements for time regions of interest
        int current_beat{1}; //A beat is completed approximately at the end of a bcl, at the end of a simulation iteration
        double RMP{}; //Resting membrane potential
        double RMP_prev{}; //For previous beat
    ...
    public:
        ...
        void process_stimulus(double);
        const atrial_configuration &get_config() {return config;}
        const double &get_time(){return time;}
        void IP3_ODEs();
        void cam_ODEs();
        ...
        double IK1_channel();
        double IKCa_channel();
        double background_currents(const int);
        double INCX_channel();
        double LCC_channel();
        void LCC_rates(const double *, double *);
        void Na_concentrations();
        void classic_Ca_handling();
        void IP3R_rates(const double *, double *);
        double RyR_rel(const double *, double *);
        double RyR_leak(const double *);
        double SERCA(const double *);
        void Euler_method();
        void output_data();
        void end_simulation();
...
void setup_simulation(atrial_cell &, const atrial_configuration &);
