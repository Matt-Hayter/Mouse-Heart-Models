...
void atrial_cell::IP3_ODEs()
{
    ...
    if (time > config.IP3_TIME) {
        if (!config.IP3_dynamic_on) {
            ec->IP3_state = config.IP3_fixed; //No IP3 dynamic, set it to a fixed value
            ecR->IP3_state = 0; //So the state remains constant
        } else { //IP3 is dynamic, so give ODEs
            double L = config.ET_1; //Ligand stimulation concentration, [uM]
